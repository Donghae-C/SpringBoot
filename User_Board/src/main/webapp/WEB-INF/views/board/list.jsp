...
<caption><h4 style="text-align: center;">자유게시판</h4></caption>
	<form method="get" id="sizeForm" action="${pageContext.request.contextPath}/board/list">
        <input type="hidden" id="hiddenSize" name="pageSize" value="${pageSize}">
        <select id="pageSize" class="form-control">
            <option value="10">페이지 사이즈 선택</option>
 ...
 <nav class="pagination-container">
	...
	<c:set var="doneLoop" value="false"/>
		 ...
		 <c:if test="${(startPage-blockCount) > 0}"> <!-- (-2) > 0  -->
		      <a class="pagination-newer" href="${pageContext.request.contextPath}/board/list?nowPage=${startPage-1}&pageSize=${pageSize}">PREV</a>
		  </c:if> 
		...
		<span class="pagination-inner"> 
		  ...
		  <c:if test="${not doneLoop}" >
		         <a class="${i==nowPage?'pagination-active':page}" href="${pageContext.request.contextPath}/board/list?nowPage=${i}&pageSize=${pageSize}">${i}</a>
		  </c:if>
		 ...
		 <c:if test="${(startPage+blockCount)<=pageList.getTotalPages()}">
		     <a class="pagination-older" href="${pageContext.request.contextPath}/board/list?nowPage=${startPage+blockCount}&pageSize=${pageSize}">NEXT</a>
		 </c:if>
...
<script>
    document.getElementById("pageSize").addEventListener("change", function(e) {
        document.getElementById("hiddenSize").value = e.target.value;
